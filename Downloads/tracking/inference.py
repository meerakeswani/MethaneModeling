...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for e in eliminationOrder:
            current, temp = joinFactorsByVariable(current, e)
            if len(temp.unconditionedVariables()) != 1:
                elim = eliminate(temp, e)
                current.append(elim)
                
        joined = normalize(joinFactors(current))
        return joined
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if jailPosition == ghostPosition:
            return 0 if noisyDistance != None else 1
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()#iterate your updates over the variable self.allPositions
        for p in self.allPositions:#inference problem: weight = P(evid t+1 | x t+1), multiply this by the belief
            weight = self.getObservationProb(noisyDistance, pacmanPosition, p, jailPosition)
            self.beliefs[p] *= weight #update self.beliefs
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        tempBelief = DiscreteDistribution() #temporary beliefs
        for oldPos in self.allPositions: #positions in the previous time step
            transProb = self.getPositionDistribution(gameState, oldPos) #distr of possible positions where ghost can end up in prev time step
            for pos in self.allPositions: #positions in the new time step
                tempBelief[pos] += transProb[pos] * self.beliefs[oldPos] #update belief for that pos in current time step
        self.beliefs = tempBelief#need to sum up over all the possible old positions (multiple) the belief of the old position (= self.beliefs) times#the trans prob of moving to that new position (newPosDist)#for every previous position pac can be, calc trans prob to new distr and add that prob to new dist
        "*** END YOUR CODE HERE ***"
